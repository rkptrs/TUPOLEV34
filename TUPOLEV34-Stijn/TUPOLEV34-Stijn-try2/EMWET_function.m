...
function [] = EMWET_function(x_1, cl_distribution, cm_distribution, Y_distribution, chord_distribution, W_TO_max, W_fuel, n_max, S, x_section, y_section, z_section, c_section, rho, V)

W_ZFW = (W_TO_max - W_fuel)/9.80665; %kg

% EMWET 
fid = fopen('TUPOLEVaircraft.init', 'wt');
    fprintf(fid,'%g %g\n', W_TO_max/9.80665, W_ZFW);
    fprintf(fid,'%g\n', n_max);
    fprintf(fid,'%g %g %g %g\n', S, x_1(3)*2, 3, 2);
    fprintf(fid,'%g %s\n', 0, 'airfoilroot');
...
fidi = fopen('TUPOLEVaircraft.load', 'wt');
    for i = 1:length(lift)
        fprintf(fidi,'%g %g %g\n', Y_distribution(i)/x_1(3), lift(i), moment(i));
    end
fclose(fidi);
